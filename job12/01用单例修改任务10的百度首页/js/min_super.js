F.module("common/result_page", function(c, b, a) {
...
F.module("superman:common/image_lazy_load", function(c, f, m) {
    var e = m.base;
    var l = 0;
    var k = 0;
    var h = [];
    ...
    f.scanAndDoRender = function(o, q) {
        ...
        var p = [];
        $.each(o.getElementsByTagName("img"), function(r, s) {
    ...
    var g = function() {
        $.each(["resize", "scroll", "load", "cardrender"], function(n, o) {
            $(window).bind(o, function(p) {
...
F.module("superman:lib/xpath_log", function(b, c, j) {
    ...
    var f = function(l, m, k, n) {
        ...
        e[k] = setTimeout(function() {
            ...
            for (var p = 0; p < m.length; p++) {
                t["xindex" + p] = m[p].replace(/[A-Za-z]/g, "");
                (t["xindex" + p] == "") && (t["xindex" + p] = "1")
            }
            var s = 3,
                v = l;
...
F.module("superman:lib/carditem_log", function(c, e, j) {
    var f = "page-card-tpl-item";
    var a = ["s_ctner_contents"];
    var g = {};
    var d = function(l, k) {
        g[k] && clearTimeout(g[k]);
        g[k] = setTimeout(function() {
            if (l.rid != undefined) {
                j.fire("cardItemLog", l)
            }
        }, 300)
    };
    var b = function(m) {
        var o = $(m),
            ...
            s = false;
        extra = "";
        while (o.attr("data-rid") == undefined && o.attr("id") != "s_ctner_contents") {
            if (o.attr("data-click") != undefined) {
                s = true;
                r = o.attr("data-click").toLowerCase();
                v = m.nodeName.toLowerCase();
                if (r == "log_link") {
                    if (o[0] && o[0].nodeName && o[0].nodeName.toLowerCase() == "a") {
                        w = $.trim(o.attr("data-title")) || $.trim(o.attr("title")) || $.trim(o.text());
                        var A = $.trim(o.attr("href"));
                        if (!A.indexOf("#") == 0 && !A.indexOf("javascript") == 0) {
                            k = A
                        }
                    }
                    if (v != "img") {
                    ...
                    if (r.indexOf("log_btn") != 1) {
                        n = r.replace("log_btn_", "");
                        r = "btn";
                        k = "";
                        w = $.trim(o.attr("data-title")) || $.trim(o.attr("title")) || $.trim(o.text())
                    }
            ...
            o = o.parent();
            if (o.attr("data-extra")) {
                extra = o.attr("data-extra")
            }
        }
        var p = {
            logactid: f,
        ...
        p.rid = s ? o.attr("data-rid") : undefined;
        var y = o.attr("data-log"),
            l = [];
        if (y) {
            y = y.replace(/\;$/g, "");
            l = y.split(";");
...
F.module("superman:ps/sindex", function(f, d, b) {
    var g = b.base;
    d.init = function() {
        var x = window,
            v = document,
        ...
        b.listen("superman:skin/skin_rewrite", "skin_change", function(k) {
            p(r, true)
        });
        try {
            if (typeof window._sp_async == "undefined") {
...
bds.se.sugsync = function() {
    ...
    var D = (function() {
        ...
        function R(ad) {
            ...
            O();
            (new Image()).src = $.url.escapeSSL("http://sclick.baidu.com/w.gif?fm=suggestion&title=%B9%D8%B1%D5&t=") + new Date().getTime()
        }
...
F.module("superman:page/analyse", function(d, c, a) {
...
$(document).ready(function() {
    ...
    F.use("superman:ps/sindex", function(f) {
        f.init()
    });
    if (typeof window._sp_async == "undefined") {
        F.use("superman:ps/sug", function(f) {
            f.init()
        })
    }
    F.use("superman:ps/log", function(f) {
        f.bindBox("nv");
        f.bindBox("u_sp");
    ...
    if ($("#toipad")[0]) {
        F.use("superman:page/toipad", function(f) {
            f.init()
        })
    }
    ...
    if (_manCard.asynJs.length > 0) {
        $.each(_manCard.asynJs, function(f, g) {
            setTimeout(function() {
    ...
    function e() {
        var f = $("#s_mod_weather");
        f.one("mouseenter", function() {
            var g = $(".city-wather .show-city-name").attr("data-key");
            $.ajaxget(s_domain.baseuri + "/other/data/weatherInfo?city=" + encodeURI(g), function(h) {
                if (h.errNo == 0 && h.data) {
                    if (h.data.weather) {
                        F.use("superman:weather/weather_tpl", function(j) {
                            j.init(h.data.weather)
                        });
                        F.use("weather/weather_ctrl", function(j) {
    ...
    if ($.isIE6) {
        $(window).bind("resize load", function(f) {
    ...
    if (location.href.indexOf("frm=resultPage") > -1 && location.href.indexOf("showType=showSkin") > -1) {
        ...
        F.use("skin/skin_init", function(f) {
            f.init()
        })
    }
    if (location.hash.indexOf("activity=wow") > -1) {
        window.location.hash = "##";
        F.use("skin/skin_cartoonExtra", function(g) {
            var f = new g.CartoonSkinExtra();
            f.init()
        })
    }
    ...
    function a() {
        $("#s_menu_mine").on("click", function() {
            ...
            F.use("superman:common/user_attr", function(f) {
                f.setAttr("supermanNavGuide", "off")
            })
        })
    }
    if (s_session.isFesLogo) {
        $("#s_mp area").on("mousedown", function(f) {
            ns_c({
                fm: "tab",
                tab: "felogo",
...
F.module("superman:page/scroll", function(b, f, p) {
...
F.module("superman:guide/superman_guide", function(d, f, k) {
    ...
    f.init = function() {
        F.use("superman:common/user_attr", function(n) {
    ...
    function e() {
        ...
        F.use("superman:common/user_attr", function(m) {
