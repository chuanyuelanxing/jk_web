...
function refreshNews($type) {
    ...
    $.ajax({
        ...
        // type: 'POST',
        url:'/news', /*/news在这里不是目录的意思，在express内只是一个路由。如http://localhost:3000/news  */
        type:'GET',
        dataType: 'json',
        data:{newstype:$type},
        success: function(data) {
            ...
            $.each(data, function(index, val) { //index为键名或者序列号，val为值
                ...
                //momet为admin.html等加载的moment.js文件
                 var $newsTime = $('<span></span>').addClass('newstime').html(moment(val.newstime).format('YYYY-MM-DD HH:mm:ss')).appendTo($p);
